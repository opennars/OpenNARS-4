...
from .branch_list import BranchList, Any, Node
...
class SparseLUT:
    ...
    def add(self, indices: list | tuple, value):
    ...
    def dump(self, root_path: str):
        with open(Path(root_path)/'LUT.pkl', 'wb') as f:
            pickle.dump((self.data, self.lut), f)
    
    def load(self, root_path: str):
        with open(Path(root_path)/'LUT.pkl', 'rb') as f:
