...
class Statement(Term):
    ...
    def clone(self):
        ...
        clone.vars_independent = IndexVar() # self._vars_independent.clone()
        clone.vars_dependent = IndexVar() # self._vars_dependent.clone()
        clone.vars_query = IndexVar() # self._vars_query.clone()
        for idx in (clone.subject.vars_independent, clone.predicate.vars_independent): clone.vars_independent.connect(idx, True)
        for idx in (clone.subject.vars_dependent, clone.predicate.vars_dependent): clone.vars_dependent.connect(idx, True)
        for idx in (clone.subject.vars_query, clone.predicate.vars_query): clone.vars_query.connect(idx, True)
