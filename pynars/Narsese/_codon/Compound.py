...
class Compound(Term):
    __slots__ = (
        '_terms', '_is_commutative', '_is_higher_order', '_is_single_only', '_is_double_only', '_is_multiple_only',
    )
    ...
    def __init__(self, connector: Connector, *terms: Term, is_input=False) -> None:
        ...
        connector, self._terms = self.prepocess_terms(connector, terms, is_input) # the connector may be changed, for example, (|, {A}, {B}) is changed into {A, B}. 
    ...
    def _terms_to_word(self, connector, *terms: Term):
    ...
    def repr(self):
        ...
        return self._terms_to_word(self.connector, *word_terms)
