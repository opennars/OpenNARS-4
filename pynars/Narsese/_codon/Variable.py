...
class Variable(Term):
    ...
    def __init__(self, prefix: VarPrefix, word: str, idx: int=0, do_hashing=False) -> None:
        ...
        # if idx is not None:
        if self.is_ivar: self.vars_independent.add(IntVar(int(idx)), [])
        if self.is_dvar: self.vars_dependent.add(IntVar(int(idx)), [])
        if self.is_qvar: self.vars_query.add(IntVar(int(idx)), [])
    ...
    def repr(self):
        ...
        if self.is_ivar:
            var = self.vars_independent.indices[0]
        elif self.is_dvar:
            var = self.vars_dependent.indices[0]
        elif self.is_qvar:
            var = self.vars_query.indices[0]
    ...
    def clone(self) -> Type['Variable']:
        clone = copy(self)
        clone.vars_independent = clone.vars_independent.clone()
        clone.vars_dependent = clone.vars_dependent.clone()
        clone.vars_query = clone.vars_query.clone()
        return clone