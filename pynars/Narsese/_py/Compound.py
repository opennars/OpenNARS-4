...
class Compound(Term):  # , OrderedSet):
    ...
    def __init__(self, connector: Connector, *terms: Term, is_input = False) -> None:
        ...
        self.connector, self._terms = self.prepocess_terms(connector, terms,
                                                           is_input)  # the connector may be changed, for example, (|, {A}, {B}) is changed into {A, B}.
    ...
    def prepocess_terms(self, connector_parent: Connector, terms: Iterable[Union[Type['Compound'], Term]],
        ...
        if self.is_commutative:
            ...
            ((term0 for term0 in term.terms) if term.is_compound and (term.connector is connector_parent) else (term,))
    ...
    def repr_with_var(self, index_var: IndexVar, pos: list):
        compound: Set[Term] = self
        word_terms = (str(component) if not component.has_var else component.repr_with_var(index_var, pos + [i]) for
                      i, component in enumerate(compound))
    ...
    @classmethod
    def ExtensionalImage(cls, term_relation: Term, *terms: Term, idx: int = None,
                         compound_product: Type['Compound'] = None) -> Type['Compound']:
        if compound_product is not None:
            # if idx is None:
            terms = compound_product.terms
            idx = terms.index(term_relation) if idx is None else idx
            compound = Compound.ExtensionalImage(term_relation, *(tm if i != idx else place_holder for i, tm in
                                                                  enumerate(compound_product)))
    ...
    @classmethod
    def IntensionalImage(cls, term_relation: Term, *terms: Term, idx: int = None,
                         compound_product: Type['Compound'] = None, compound_image: Type['Compound'] = None) -> Type[
        'Compound']:
        if compound_product is not None:
            # if idx is None:
            idx = compound_product.terms.index(term_relation) if idx is None else idx
            compound = Compound.IntensionalImage(term_relation, *(tm if i != idx else place_holder for i, tm in
                                                                  enumerate(compound_product)))
        elif terms is not None:
            if idx is not None:
                terms: list = [*terms[:idx], place_holder, *terms[idx:]]
    ...
    def Image(cls, replaced_term: Term, compound_image: Type['Compound'], idx_replaced: int = None) -> Type['Compound']:
        ...
        compound = Compound(compound_image.connector,
                            *((place_holder if i == idx_replaced else tm if tm != place_holder else replaced_term) for
                              i, tm in enumerate(compound_image)))
    ...
    def Product(cls, term: Term, *terms: Term, idx: int = None, compound_image: Type['Compound'] = None):
        if compound_image is not None:
            ...
            compound = Compound(Connector.Product, term, *terms)
        return cls._convert(compound)
    ...
    def IntensionalIntersection(cls, *terms: Union[Term, Type['Compound']]) -> Type['Compound']:
        terms = (term for compound in terms for term in (
            compound if compound.is_compound and compound.connector == Connector.IntensionalIntersection else (
            compound,)))
    ...
    def ExtensionalIntersection(cls, *terms: Union[Term, Type['Compound']]) -> Type['Compound']:
        terms = (term for compound in terms for term in (
            compound if compound.is_compound and compound.connector == Connector.ExtensionalIntersection else (
            compound,)))
