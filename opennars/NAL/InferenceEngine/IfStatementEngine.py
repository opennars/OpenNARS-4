...
from opennars.Narsese import Task, Sentence, Question, Term, Compound, Statement, Goal, Judgement, Copula, Connector, Budget

def inference(task: Task, belief: Sentence, common_term: Term, budget_tasklink: Budget, budget_termlink: Budget, *args, **kwargs) -> list[Task]:
    '''
    Interface for the inference engine
    '''
    if task.term == belief.term:
        return []
    return do_semantic_inference_two_judgment(task, belief, budget_tasklink, budget_termlink)
...
def do_semantic_inference_two_judgment(j1_task: Task, j2: Sentence,
                                       budget_tasklink: Budget = None, budget_termlink: Budget = None) -> list[Task]:
    """
    ...
    if isinstance(j1.term, Statement) and isinstance(j2.term,Statement) and \
        ...
        assert (isinstance(j1.term, Statement) and not j1.term.copula.is_first_order), "ERROR"
