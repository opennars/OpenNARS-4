...
from opennars.NAL.Functions.BudgetFunctions import Budget_evaluate_goal_solution
from opennars.NAL.Functions.Tools import calculate_solution_quality
from opennars.NAL.Functions.Tools import project, project_truth
from opennars.NAL.Functions.Tools import revisible
from opennars.NAL.Inference import local__revision
from opennars.NAL.Inference.LocalRules import solution_query, solution_question
from opennars.NAL.MetaLevelInference.VariableSubstitution import get_elimination__var_const
from opennars.NARS.DataStructures._py.Link import TaskLink
from opennars.NARS.GlobalEval import GlobalEval
from opennars.Narsese import Statement, Budget, Task
from opennars.Narsese._py.Sentence import Goal, Question
from opennars.Narsese._py.Task import Belief, Desire
from .Bag import Bag
        ...
        if truth.e > Config.Td_decision_threshold:
            if (task is not None) and task.is_operation and not (desire is not None and desire.evidential_base.contains(task.evidential_base)):
                    ...
                    if stat.is_atom:
                        ...
                        op = stat.predicate
                    from opennars.NARS.Operation.Register import registered_operators
                    from opennars.NARS.Operation.Execution import execute
